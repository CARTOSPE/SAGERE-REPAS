...
Version Web (Streamlit) + persistance Google Sheets
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# Structure du Google Sheet (4 onglets) :
#   "menus"    : col A = clé semaine (ex: 2026-S27), col B = JSON menu complet
#   "commandes": col A = clé semaine,                col B = JSON {salarié:{jour:{cat:[...]}}}
#   "config"   : col A = "salaries" | "carte",       col B = JSON valeur
...
def get_sheet(tab):
    ...
    try:
        ...
        ws = sh.add_worksheet(title=tab, rows=500, cols=3)
        return ws

def gs_load_all(tab):
    """→ dict {clé: valeur_json}"""
    try:
...
def gs_save(tab, key, value):
    """Écrit ou met à jour la ligne (key, json(value))."""
    try:
        ws   = get_sheet(tab)
...
# Fonctions métier
def load_menus():
    data = gs_load_all("menus")
    if not data:
        wk = week_key()
        data = {wk:{"semaine":wk,"periode":"","jours":{j:{c:[] for c in CAT_MENU} for j in JOURS}}}
    return data

def save_menu(wk, menu):        gs_save("menus",    wk,          menu)
def load_commandes():           return gs_load_all("commandes")
def save_commandes_wk(wk, c):  gs_save("commandes", wk,          c)
def load_salaries():
    v = gs_load_all("config").get("salaries")
    return v if v else DEFAULT_SALARIES
def save_salaries(s):           gs_save("config",   "salaries",   s)
def load_carte():
    v = gs_load_all("config").get("carte")
    return v if v else DEFAULT_CARTE
def save_carte(c):              gs_save("config",   "carte",      c)
    ...
    if len(tables) >= 25:
        for ci, cat in enumerate(CAT_MENU):
            for ji, jour in enumerate(JOURS):
                t = tables[ci*5+ji]
                jours_data[jour][cat] = [tr.get_text(strip=True) for tr in t.find_all("tr") if tr.get_text(strip=True)]
...
def inline_cat_label(color, icon, cat):
...
def build_export_traiteur(menu, commandes, salaries, carte, periode="", wk=""):
    ...
    def write_block(cat_key, label, items_fn, cmd_cat):
        ...
        for item in all_items:
            ...
            for col,val,fnt,bg in [
                (1,label,Font(name="Calibri",size=8,color="667090"),"1E2240"),
                (2,item, Font(name="Calibri",size=10,color="D8DCFF"),"1E2240"),
            ]:
                c=ws.cell(row=row[0],column=col,value=val); c.font=fnt; c.fill=fill(bg); c.border=thin
...
# ──────────────────────────────────────────────────────────────────────────────
# Init session state — charge Google Sheets une seule fois par session
# ──────────────────────────────────────────────────────────────────────────────
def init_state():
...
with st.sidebar:
    ...
    for key, label in {
        ...
        "carte":"🗂 Carte permanente","salaries":"👥 Salariés","admin":"📊 Exports & Admin"
    }.items():
    ...
    sel = st.selectbox("",labels,index=wks.index(st.session_state.week_sel),
    ...
    if st.session_state.page == "commande":
        ...
        st.session_state.salarie = st.selectbox("",st.session_state.salaries,index=sal_idx,
        ...
        sem = st.session_state.commandes.get(st.session_state.week_sel,{}).get(st.session_state.salarie,{})
        for jour in JOURS:
            total = sum(len(v) for v in sem.get(jour,{}).values())
            if total:
                st.markdown(f'<span class="recap-ok">✓ {jour[:3]}</span> — {total} article(s)',unsafe_allow_html=True)
            else:
...
# ══════════════════════════════════════════════════════════════════════════════
if st.session_state.page == "commande":
    sal=st.session_state.salarie; jour=st.session_state.jour; wk=st.session_state.week_sel
    menu=st.session_state.menus.get(wk,{})
    c1,c2=st.columns([2,3])
    with c1: st.markdown(f"# {jour}")
    with c2: st.markdown(f"<br><span style='color:#8890C0'>{week_label(wk)}</span>",unsafe_allow_html=True)
    ...
    if not has_menu and not has_carte:
        ...
        for cat in CAT_MENU:
            ...
            color=CAT_COLORS[cat]; icon=CAT_ICONS.get(cat,""); selected=existing.get(cat,[])
            st.markdown(cat_header(color,cat.upper(),icon),unsafe_allow_html=True)
            cols=st.columns(2)
            for i,item in enumerate(items):
                with cols[i%2]:
                    if st.checkbox(item,value=(item in selected),key=f"cb_{wk}_{sal}_{jour}_{cat}_{item}"):
        ...
        if has_carte:
            ...
            for sub in CAT_MENU:
                ...
                st.markdown(cat_header(color,f"↳ {sub.upper()} (carte)",icon),unsafe_allow_html=True)
                cols=st.columns(2)
                for i,item in enumerate(items):
                    with cols[i%2]:
                        if st.checkbox(item,value=(item in selected),key=f"cb_{wk}_{sal}_{jour}_{cmd_key}_{item}"):
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    menu=st.session_state.menus.get(wk,{"semaine":wk,"periode":"","jours":{j:{c:[] for c in CAT_MENU} for j in JOURS}})

    st.markdown("### 📥 Importer le fichier du traiteur")
    uploaded=st.file_uploader("Fichier `.xls` reçu du traiteur",type=["xls","html","htm"],key="uploader_menu")
    if uploaded:
        try:
            parsed=parse_traiteur_html(uploaded.read()); wk_imp=parsed["semaine"]
            ...
            if st.button("✅ Confirmer l'import",type="primary"):
                st.session_state.menus[wk_imp]=parsed; save_menu(wk_imp,parsed)
    ...
    for t,jour in zip(tabs,JOURS):
        with t:
            new_jours[jour]={}; jour_data=menu.get("jours",{}).get(jour,{})
            for cat in CAT_MENU:
                color=CAT_COLORS[cat]; icon=CAT_ICONS.get(cat,"")
                st.markdown(inline_cat_label(color,icon,cat),unsafe_allow_html=True)
                val="\n".join(jour_data.get(cat,[]))
                txt=st.text_area("Un article par ligne",value=val,height=110,
                                  key=f"menu_{wk}_{jour}_{cat}",label_visibility="collapsed")
    ...
    if st.button("💾 Enregistrer le menu",type="primary"):
        ...
        st.session_state.menus[wk]=m; save_menu(wk,m)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for t,cat in zip(tabs,CAT_MENU):
        with t:
            ...
            st.markdown(inline_cat_label(color,icon,cat),unsafe_allow_html=True)
            val="\n".join(carte.get(cat,[]))
            txt=st.text_area("Un article par ligne",value=val,height=200,
    ...
    with c1:
        if st.button("💾 Enregistrer la carte",type="primary",use_container_width=True):
            st.session_state.carte=new_carte; save_carte(new_carte)
            st.success(f"Carte enregistrée — {sum(len(v) for v in new_carte.values())} article(s)."); st.rerun()
    with c2:
        if st.button("↺ Réinitialiser",use_container_width=True):
            st.session_state.carte=dict(DEFAULT_CARTE); save_carte(DEFAULT_CARTE)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    for i,sal in enumerate(salaries):
        ...
        with c1: salaries[i]=st.text_input(f"Salarié {i+1}",value=sal,key=f"sal_edit_{i}",label_visibility="collapsed")
        with c2:
            if st.button("✕",key=f"sal_del_{i}"):
                salaries.pop(i); st.session_state.salaries=salaries; save_salaries(salaries); st.rerun()
    st.divider()
    new_sal=st.text_input("➕ Ajouter un salarié",placeholder="Prénom NOM",key="new_sal_input")
    if st.button("Ajouter",type="primary") and new_sal.strip():
        salaries.append(new_sal.strip()); st.session_state.salaries=salaries; save_salaries(salaries); st.rerun()
    if st.button("💾 Enregistrer les modifications",use_container_width=True):
        ...
        save_salaries(st.session_state.salaries); st.success("Liste mise à jour."); st.rerun()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    wk=st.session_state.week_sel; menu=st.session_state.menus.get(wk,{})
    commandes=st.session_state.commandes.get(wk,{})
    salaries=st.session_state.salaries; carte=st.session_state.carte
    periode=menu.get("periode","").strip() or week_label(wk)
    ...
    if not commandes:
        ...
        recap=[{"Salarié":sal,**{jour:f"✓ {sum(len(v) for v in commandes.get(sal,{}).get(jour,{}).values())}"
                                  if sum(len(v) for v in commandes.get(sal,{}).get(jour,{}).values()) else "—"
                                  for jour in JOURS}} for sal in salaries]
        st.table(recap)
    st.markdown("---")
        ...
        if st.button("Créer",type="primary") and new_wk.strip():
            ...
            if re.match(r'\d{4}-S\d{2}',nwk):
                if nwk not in st.session_state.menus:
                    m={"semaine":nwk,"periode":"","jours":{j:{c:[] for c in CAT_MENU} for j in JOURS}}
                    st.session_state.menus[nwk]=m; save_menu(nwk,m)
                st.session_state.week_sel=nwk; st.success(f"Semaine {week_label(nwk)} créée."); st.rerun()
            else: st.error("Format : 2026-S30")
    with c2:
        st.markdown("**Semaines disponibles :**")
        for k in weeks_list(st.session_state.menus):
            n=sum(sum(len(v) for v in st.session_state.commandes.get(k,{}).get(s,{}).values()) for s in salaries)
            st.markdown(f"- `{k}` — {week_label(k)} — {n} article(s)")
    st.markdown("---")
    st.markdown("### 🔄 Recharger depuis Google Sheets")
    if st.button("🔄 Recharger les données",use_container_width=True):
        del st.session_state["loaded"]; st.rerun()
